...
- ✅ 结构化输出 + Pydantic 校验。
- ✅ `LLM_MAX_ATTEMPTS` 有限重试、常见模型字段别名归一化、非 JSON 错误转为结构化 API 响应。
- ✅ 海报服务支持 Mock/真实端点切换。
- ⬜ 模型回退、配额、缓存和线上评估仍需补充。
...
- 输出清理代码围栏后用 Pydantic 校验；常见字段别名会先归一化，并按配置进行有限重试，最终失败才终止。
...
| Prompt | 节点 / 用途 |
| --- | --- |
| `PARSE_USER_INPUT_SYSTEM` | chat_graph 四阶段需求收集、注意事项询问与确认门禁 |
...
| `QUALITY_REVIEW_SYSTEM` | quality_review；行程天数不符必须列为 blocking issue |

## 5. 结构化输出与失败处理

涉及后续计算的数据必须使用 Pydantic 校验。模型调用失败会在有限次数内重试；最终异常由 API 转成 JSON `502` 或 SSE `error`，前端使用安全解析器展示用户可读原因，不再把纯文本 `Internal Server Error` 当 JSON 解析。

| 节点 | 失败行为 |
| --- | --- |
| retrieve_resources | 资源充实失败 → 抛出异常，工作流终止 |
| plan_itinerary | 交通估算 / 行程编排失败 → 抛出异常，工作流终止 |
| calculate_quote | 过滤零金额项；全零时使用非零保守估算；其他失败抛异常 |
| quality_review | 抛出异常（无 LLM 无法审核；Mock 模式下同样抛错） |

仅测试用的 `MOCK_MODEL_MODE=true` 走确定性路径，不调用外部 API。
...
- 行程节奏、主题和描述：可由模型生成。
- 硬性约束：由确定性规则引擎（validate_constraints + verifier）检查，特别是请求天数必须完整覆盖。
...
- 真实模式按普通 GPU 队列能力顺序生成 1 张封面 + 每天 1-3 张插图，避免排队时间误算成执行超时；`MOCK_IMAGEGEN=true` 时使用预处理素材。
- **封面 = 整体规划**：Brief 取产品标题、目的地、客群，视觉元素优先取必去地标（`must_visit`）或主题（`themes`），比例 3:4。
- **分日插图 = 当天行程**：主图取 `day.theme`，副图（第 2/3 张）叠加当天活动标题高亮，比例 4:3（适配横版 PDF）。
- 封面 ComfyUI 失败时尝试真实资源图片或高德静态地图；每日图若仍不完整，`poster_route` 阻止审批与交付。
...
IMAGEGEN_QUEUE_TIMEOUT_SECONDS=900
IMAGEGEN_EXECUTION_TIMEOUT_SECONDS=1200
```
