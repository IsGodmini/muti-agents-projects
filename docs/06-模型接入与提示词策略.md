...
| `LLM_MODEL_COMPLEX` | `DeepSeek-V4-Pro` | 火山引擎 Ark | 需求解析、交通估算、行程编排、成本核算、质量审核、修复建议 |
...
| `ITINERARY_ENRICHMENT_SYSTEM` | 分日主题与活动描述（备用） |
...
- 真实模式会生成 1 张封面 + 每 1 张分日插图；`MOCK_IMAGEGEN=true` 时直接返回预处理素材，不调用真实端点。
